...
<img src="docs/pico-display-gpio.svg" alt="Verdrahtung von Display, Pico, Relais und Rückmeldungen" width="62.5%">
...
`-dirty` bedeutet: exakter Build-Stand liegt in **keinem** Commit; `71704ee` ist nur
  ...
  **Achtung `-dirty`:** Diese Firmware wurde aus einem Arbeitsstand mit nicht eingecheckten Aenderungen gebaut; der genannte Commit ist dann nur die naechstliegende Basis und liegt nicht 1:1 auf GitHub. Nur Versionen **ohne** 
...
Rueckverfolgung zu GitHub: 

Der Teil `g<hash>` ist der kurze Git-Commit-Hash (das fuehrende `g` = „git"). Beispiel `05.00019.g71704ee` → Commit `71704ee`. 
