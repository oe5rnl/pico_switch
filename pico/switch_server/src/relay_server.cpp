...
static std::string build_scenes_html(const Session *session) {
  ...
  std::string html = "<!DOCTYPE html><html lang=\"de\"><head><meta charset=\"UTF-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>" + html_escape(site_title) + " - Szenen</title><style>html,body{margin:0}body{box-sizing:border-box;font-family:'Segoe UI',sans-serif;background:#121212;color:#e0e0e0;min-height:100vh;padding:108px 24px 24px}" + page_header_css() +
      ...
      "<script>const conn=document.getElementById('conn');let lastSseActivity=0;let lastConnectAttempt=0;let es=null;let reconnectTimer=0;function markConn(ok){if(!conn)return;conn.textContent=ok?'Verbunden':'Getrennt';conn.className=ok?'ok':'err'}function noteSseActivity(){lastSseActivity=Date.now();markConn(true)}function scheduleReconnect(delay=1000){if(reconnectTimer)return;reconnectTimer=setTimeout(()=>{reconnectTimer=0;connectEvents()},delay)}function forceReconnect(){if(es){es.close();es=null}scheduleReconnect(0)}function connectEvents(){if(es)es.close();lastConnectAttempt=Date.now();es=new EventSource('/events');es.onopen=()=>{noteSseActivity()};es.onerror=()=>{markConn(false);if(es){es.close();es=null}scheduleReconnect()};es.addEventListener('ping',()=>{noteSseActivity()});es.onmessage=e=>{noteSseActivity();try{const d=JSON.parse(e.data);const m=document.getElementById('mode');if(m&&typeof d.scene_mode==='boolean'&&m!==document.activeElement)m.checked=d.scene_mode}catch(x){}}}connectEvents();window.addEventListener('pagehide',()=>{if(es)es.close()});setInterval(()=>{const now=Date.now();const activeLink=lastSseActivity&&now-lastSseActivity<2500;markConn(!!activeLink);if(!activeLink&&now-lastConnectAttempt>=2500&&!reconnectTimer)forceReconnect()},1000);"
...
static std::string build_index_html(const Session *session, bool can_control) {
  ...
  if (can_control) {
    // Leerer Container; das Grid (Buttons bzw. Szenen+Relais) wird clientseitig
    // aufgebaut und bei Moduswechsel per SSE live umgebaut (kein Reload).
    body_section = "<div id=\"content\"></div>";
  } else {
    body_section = "<div class=\"notice\"><p>Bitte <a href=\"/login?next=/\">anmelden</a>, </p></div>";
  }
  std::string html = "<!DOCTYPE html><html lang=\"de\"><head><meta charset=\"UTF-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>" + html_escape(site_title) + "</title><style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:'Segoe UI',system-ui,sans-serif;background:#0d1117;color:#c9d1d9;min-height:100vh;display:flex;flex-direction:column;align-items:center;padding:20px 16px 40px}" + page_header_css() + "h1{margin:92px 0 24px;font-size:1.4rem;color:#e6edf3;font-weight:600;text-align:center}.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:12px;width:100%;max-width:800px}.card{width:100%;background:#161b22;border:1px solid #30363d;padding:10px;display:flex;align-items:center;cursor:pointer;text-align:inherit;font:inherit;color:inherit}.card.on{border-color:#238636}.card.error{background:#5a1616;border-color:#f85149}.card.error .label,.card.error .state{color:#fff}.card.on.dirty{position:relative}.card.on.dirty::after{content:'';position:absolute;top:4px;right:4px;width:10px;height:10px;border-radius:50%;background:#f85149}.card:focus-visible{outline:2px solid #58a6ff;outline-offset:2px}.relay-row{display:flex;align-items:center;gap:10px;width:100%}.label{flex:1;font-size:.9rem;font-weight:700;color:#8b949e;text-transform:uppercase;text-align:left}.relay-num{flex:none;font-size:.65rem;color:#484f58;min-width:24px}.state{flex:none;min-width:38px;font-size:.9rem;font-weight:700;color:#8b949e;text-align:right}.card.on .state{color:#3fb950}.notice{background:#161b22;border:1px solid #30363d;padding:24px 28px;color:#c9d1d9;font-size:.95rem;text-align:center;max-width:560px}.notice a{color:#58a6ff;font-weight:700}</style></head><body data-user=\"" + html_escape(current_session_username(session)) + "\" data-can-control=\"" + (can_control ? "1" : "0") + "\">" + page_header_html(session, true) + "<h1 id=\"subtitle\">" + html_escape(site_subtitle) + "</h1>" + body_section + "<script>const initialState=" + state_json(session != nullptr || public_access) + ";const sceneData=" + scenes_config_json() + ";const canControl=document.body.dataset.canControl==='1';const conn=document.getElementById('conn');const other=document.getElementById('other-users');const currentUser=document.body.dataset.user;const content=document.getElementById('content');let names=(initialState.names&&initialState.names.length)?initialState.names:Array.from({length:8},(_,i)=>'Relais '+(i+1));let curMode=null;let sceneBtns={};let last={relays:initialState.relays||[],errors:initialState.feedback_errors||[],btnEn:initialState.btn_en||[],aScene:initialState.active_scene,sErrors:initialState.scene_errors,sDirty:initialState.scene_dirty};function makeHeader(t){const d=document.createElement('div');d.style.cssText='width:100%;max-width:800px;margin:22px 0 8px;color:#8b949e;font-size:.8rem;font-weight:700;text-transform:uppercase';d.textContent=t;return d}function activateScene(i){fetch('/scene/'+i+'/activate',{method:'POST'}).catch(()=>{})}function buildSceneGrid(sg){sceneBtns={};sceneData.forEach((s,i)=>{if(!s.en)return;const b=document.createElement('button');b.className='card';b.type='button';b.id='sc'+i;b.onclick=()=>activateScene(i);const row=document.createElement('div');row.className='relay-row';const lab=document.createElement('span');lab.className='label';lab.textContent=s.name||('Szene '+(i+1));const st=document.createElement('span');st.className='state';st.textContent='\\u25B6';row.appendChild(lab);row.appendChild(st);b.appendChild(row);sg.appendChild(b);sceneBtns[i]=b})}function buildRelayGrid(g){const be=last.btnEn||[];for(let i=0;i<8;i++){if(be[i]===false)continue;const on=!!last.relays[i];const card=document.createElement('button');card.type='button';card.className='card'+(last.errors[i]?' error':(on?' on':''));card.id='c'+i;card.onclick=()=>toggle(i);card.innerHTML=`<div class=\"relay-row\"><span class=\"relay-num\">#${i+1}</span><span class=\"label\" id=\"l${i}\">${names[i]}</span><span class=\"state\" id=\"s${i}\">${on?'ON':'OFF'}</span></div>`;g.appendChild(card)}}function applyScenes(as,errs=[],dirty=false){Object.keys(sceneBtns).forEach(k=>{sceneBtns[k].className='card'+(errs[k]?' error':((+k===as)?(' on'+(dirty?' dirty':'')):''))})}function buildLayout(mode){if(!content)return;curMode=mode;content.innerHTML='';sceneBtns={};if(mode){content.appendChild(makeHeader('Szenen'));const sg=document.createElement('div');sg.className='grid';sg.id='scenegrid';content.appendChild(sg);content.appendChild(makeHeader('Relais'));const g=document.createElement('div');g.className='grid';g.id='grid';content.appendChild(g);buildSceneGrid(sg);buildRelayGrid(g)}else{const g=document.createElement('div');g.className='grid';g.id='grid';content.appendChild(g);buildRelayGrid(g)}applyScenes(last.aScene,last.sErrors,last.sDirty)}function renderActiveUsers(users){if(!other)return;if(users===null){other.style.display='none';return}other.style.display='';const list=(users||[]).filter(u=>!currentUser||currentUser==='-'||u.username!==currentUser);if(!list.length){other.textContent='Andere Benutzer: -';other.title='Keine anderen aktiven Anmeldungen';return}const text=list.map(u=>u.username+' ('+u.remaining+' min)').join(', ');other.textContent='Andere Benutzer: '+text;other.title=text}function applyState(states,ns,users,errors=[]){if(ns)names=ns;if(states){last.relays=states;last.errors=errors}if(canControl&&states)states.forEach((on,i)=>{const lbl=document.getElementById('l'+i);if(!lbl)return;lbl.textContent=names[i];const st=document.getElementById('s'+i);const c=document.getElementById('c'+i);if(!st||!c)return;st.textContent=on?'ON':'OFF';c.className='card'+(errors[i]?' error':(on?' on':''))});renderActiveUsers(users)}function handleData(d){if(d.names)names=d.names;if(d.relays){last.relays=d.relays;last.errors=d.feedback_errors||[]}if('active_scene'in d)last.aScene=d.active_scene;if('scene_errors'in d)last.sErrors=d.scene_errors;if('scene_dirty'in d)last.sDirty=d.scene_dirty;if(typeof d.scene_mode==='boolean'&&d.scene_mode!==curMode)buildLayout(d.scene_mode);applyState(d.relays,d.names,d.active_users,d.feedback_errors);applyScenes(last.aScene,last.sErrors,last.sDirty);if(d.title){document.getElementById('sitetitle').textContent=d.title;document.title=d.title}if(d.subtitle)document.getElementById('subtitle').innerHTML=d.subtitle}function toggle(i){fetch('/relay/'+i+'/toggle',{method:'POST',credentials:'same-origin'}).then(r=>{if(r.status===401){location.href='/login?next=/';throw new Error('unauthorized')}if(!r.ok)throw new Error('HTTP '+r.status);return r.json()}).then(d=>handleData(d)).catch(()=>{conn.textContent='Fehler';conn.className='err'})}buildLayout(!!initialState.scene_mode);renderActiveUsers(initialState.active_users);let lastSseActivity=0;let lastConnectAttempt=0;let es=null;let reconnectTimer=0;function markConn(ok){conn.textContent=ok?'Verbunden':'Getrennt';conn.className=ok?'ok':'err'}function noteSseActivity(){lastSseActivity=Date.now();markConn(true)}function scheduleReconnect(delay=1000){if(reconnectTimer)return;reconnectTimer=setTimeout(()=>{reconnectTimer=0;connectEvents()},delay)}function forceReconnect(){if(es){es.close();es=null}scheduleReconnect(0)}function connectEvents(){if(es)es.close();lastConnectAttempt=Date.now();es=new EventSource('/events');es.onopen=()=>{noteSseActivity()};es.onerror=()=>{markConn(false);if(es){es.close();es=null}scheduleReconnect()};es.addEventListener('ping',()=>{noteSseActivity()});es.onmessage=e=>{noteSseActivity();try{const d=JSON.parse(e.data);handleData(d)}catch(x){}}}connectEvents();window.addEventListener('pagehide',()=>{if(es)es.close()});setInterval(()=>{const now=Date.now();const activeLink=lastSseActivity&&now-lastSseActivity<2500;markConn(!!activeLink);if(!activeLink&&now-lastConnectAttempt>=2500&&!reconnectTimer)forceReconnect()},1000);</script></body></html>";
  return html;
}
