...
// Eine Szene gilt als "aktiv" (gruen), wenn sie aktiviert ist, mindestens eine
// Aktion (!= unveraendert) hat und der aktuelle Zustand ihrer Vorgabe entspricht.
// So kann jede Szene unabhaengig aktiv sein (mehrere gleichzeitig moeglich).
static bool scene_is_active(uint8_t idx) {
  if (idx >= cfg::SCENE_COUNT || !scenes[idx].enabled) return false;
  bool any = false;
  for (uint8_t b = 0; b < cfg::MAX_BUTTONS; ++b) {
    if (scenes[idx].action[b] != 2) { any = true; break; }
  }
  return any && scene_state_matches(idx);
}
...
static std::string state_json(bool include_users) {
  ...
  for (uint8_t s = 0; s < cfg::SCENE_COUNT; ++s) {
    if (s) out += ',';
    out += scene_is_active(s) ? "true" : "false";
  }
  out += "],\"feedback_errors\":[";
  for (uint8_t b = 0; b < cfg::MAX_BUTTONS; ++b) {
    if (b) out += ',';
